...
class WarehouseSolver:
    ...
    def solve_picker_routing_parallel(self) -> None:
        ...
        def calculate_total_distance(route) :
            return sum(self.od_matrix.loc[route[i], route[i + 1]] for i in range(len(route) - 1))
        ...
        def vns_sa(route, initial_temp = 100, cooling_rate= 0.95, min_temp= 0, max_k = 3) :
            neighborhoods = [two_opt, swap, insertion]
            current = route[:]
            current_cost = calculate_total_distance(current)
            best_route = current[:]
