...
class WarehouseSolver:
    ...
    def solve_picker_routing_parallel(self) -> None:
        ...
        - vns(4way)
        """
        def greedy_init(start, locs, dist_matrix, end) :
            unvisited = set(locs)
            route = [start]
            current = start 
            while unvisited :
                next_loc = min(unvisited, key= lambda x : dist_matrix.loc[current, x])
        ...
        def vns(route, dist_matrix, max_k = 3) :
            ...
            def or_opt(route, max_block_size = 3) :
                best_route = route[:]
                ...
                while improved :
                    ...
                    for block_size in range(1, max_block_size + 1) :
                        for i in range(1, len(route) - block_size) :
                            block = best_route[i:i+block_size]
                            remaining = best_route[:i] + best_route[i + block_size :]
            ...
            neighborhoods = [or_opt, two_opt, swap, insertion]
            best_route = route[:]
        ...
        def optimize_cart_route(cart_no, cart_df, od_matrix):
            ...
            if len(locs) <= 1:
                ...
                route = greedy_init(od_matrix.index[0], locs, od_matrix, od_matrix.index[1])
                route = vns(route, od_matrix, max_k = 4)
