...
class WarehouseSolver:
    ...
    def solve_picker_routing_parallel(self) -> None:
        """피커 경로 최적화 - ACO 방식 (병렬처리)
        n_ants : 각 반복에서 경로를 탐색할 개미 수 20
        n_iterations : 반복 횟수 50
        alpha : 페로몬 중요도 1.2(경로 누적 효과)
        beta : 거리 기반 유도(가시성) 중요도 3 (가시성 - 가까운 거리 선호)
        rho : 페로몬 증발 계수 0.05 (페로몬 증발 완화 → 이전 학습 유지)
        q = 5 : 좋은 경로에 보상할 페로몬 양
        기존 : 19484.45
        """
        ...
        def aco_optimize_route(locs, dist_matrix, n_ants = 25, n_iterations= 50, alpha= 1.2, beta= 3, rho= 0.05, q= 10) :
            # 초기 설정
            all_nodes = [self.start_location] + locs + [self.end_location]
            pheromone = pd.DataFrame(
                np.ones((len(all_nodes), len(all_nodes))),
                index= all_nodes,
                columns= all_nodes
            )
            visibility = 1 / (dist_matrix.loc[all_nodes, all_nodes] + 1e-6)
            ...
            for _ in range(n_iterations) :
                ...
                for _ in range(n_ants) :
                    ...
                    while unvisited :
                        ...
                        for next_node in unvisited :
                            tau = pheromone.loc[current, next_node] ** alpha
                            eta = visibility.loc[current, next_node] ** beta 
                            probs.append((next_node, tau * eta))
                        total = sum(p for _, p in probs)
                        probs = [(node, p / total) for node, p in probs]
                        
                        next_node = np.random.choice([p[0] for p in probs], p=[p[1] for p in probs])
                        route.append(next_node)
                        unvisited.remove(next_node)
                        
                    route.append(self.end_location)
                    length = calculate_total_distance(route, dist_matrix)
                ...
                for route, length in zip(all_routes, all_lengths) :
                    for i in range(len(route) - 1) :
                        pheromone.loc[route[i], route[i + 1]] += q/length 
            
            return [node for node in best_route if node not in [self.start_location, self.end_location]]
        ...
        def optimize_cart_route(cart_no, cart_df, od_matrix):
            ...
            if len(locs) <= 1:
                ...
                route = aco_optimize_route(locs, od_matrix)
