...
class WarehouseSolver:
    ...
    def solve_picker_routing_parallel(self) -> None:
        """피커 경로 최적화 (병렬처리)
        1. 초기 집단 생성 (LOC 랜덤 배열)
        2. 세대 반복(n_gerations만큼)
        2-1. 적합도 평가(각 경로의 총 이동 거리를 계산하여 짧을수록 좋은)
        2-2. 정렬 & 상위 개체 유지(엘리트 선택)
        2-3. 교차 + 돌연변이로 자식 생성
        - 교차 : 선택된 부모 둘의 경로 일부를 섞어 새로운 자식 경로
        - 돌연변이 : 일부 자식 개체는 LOC 순서를 살짝 바꾸어 다양성 유지
        3. 세대 교체 : 상위 개체 + 새 자식 개체로 새로운 세대 구성
        4. 반복 : 만족할 만한 해가 나올 때까지 과정 반복
        population_size= 카트 내 sku 수 : 5개 > 경로 조합 5! = 120 중 20개
        """
        
        # 하나의 해 : n개의 LOC을 어떤 순서로 방문할지
        
        def ga_optimize_route(locs, dist_matrix, 
                              n_generaions= 100, population_size= 50, 
                              elite_size= 10, mutation_rate = 0.4) :
            ...
            for _ in range(n_generaions) :
                fitness_scores = [(ind, calculate_total_distance(ind)) for ind in population]
                fitness_scores.sort(key= lambda x : x[1])
                ranked_population = [ind for ind, _ in fitness_scores]
                
                new_population = ranked_population[:elite_size]
                
                while len(new_population) < population_size :
                    parent_idxs = np.random.choice(min(10, len(ranked_population)), 2, replace= False)
                    parent1, parent2 = ranked_population[parent_idxs[0]], ranked_population[parent_idxs[1]]
        ...
        def optimize_cart_route(cart_no, cart_df, od_matrix):
            ...
            if len(locs) <= 1:
                ...
                route = ga_optimize_route(
                    locs, od_matrix, n_generaions= 40, population_size= 20,
                    elite_size= 5, mutation_rate= 0.2
                )
            seq_records = []
            seq = 1
            for loc in route:
