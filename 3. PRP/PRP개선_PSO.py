...
class WarehouseSolver:
    ...
    def solve_picker_routing_parallel(self) -> None:
        ...
        """피커 경로 최적화 (병렬처리)
        입자(LOC 방문 순서) 군집 최적화
        자신의 최적해와 전체 무리의 치적해를 참고하여 좋은 해를 찾아감
        Particle : LOC 방문 순서
        Velocity : 순열 간 차이 ex. swap 연산 리스트
        Position : 현재 LOC 순서
        p_best : 각 입자의 과거 중 가장 좋은 경로
        g_best : 전체 입자 중 가장 짧으 경로
        """

        def pso_optimize_route(locs, dist_matrix, n_particles= 50, n_iterations= 130, 
                               w = 0.3, c1 = 1.8, c2= 2.2) :
        # w : 관성, c1 : 개인 최적 c2 : 전체 최적
            ...
            particles = [list(np.random.permutation(locs)) for _ in range(n_particles)]
            ...
            g_best_scores = min(p_best_scores)
