...
class WarehouseSolver:
    ...
    def solve_picker_routing_parallel(self) -> None:
        ...
        def tsp_optimize_locs(locs, od_matrix, start_loc, end_loc, time_limit_sec = 50) :
            all_nodes = [start_loc] + locs + [end_loc]
            ...
            for i, from_node in enumerate(all_nodes) :
                for j, to_node in enumerate(all_nodes) :
                    distance_matrix[i][j] = od_matrix.loc[from_node, to_node]
            
            manager = pywrapcp.RoutingIndexManager(size, 1, 
                                                   [node_index[start_loc]], 
                                                   [node_index[end_loc]])
            ...
            if solution :
                ...
                return [n for n in route if n not in [start_loc, end_loc]]
            else :
                return locs        
...
#  ✅ 총 피킹 시간: 19561.68초 (피킹 1426회, 이동 거리 15283.68)